...
class StaticPageContentC extends Controller
{
  protected $page_route;
  public function __construct()
  {
    $this->page_route = 'static-page-contents';
  }
  public function index($id = null)
  {
    $authors = Author::all();
    $page_no = $_GET['page'] ?? 1;
    $rows = StaticPageContent::get();
    if ($id != null) {
      $sd = StaticPageContent::find($id);
      if (!is_null($sd)) {
        $ft = 'edit';
    ...
    $page_title = "Static Page Contents";
    $page_route = $this->page_route;
    $data = compact('rows', 'sd', 'ft', 'url', 'title', 'page_title', 'page_route', 'page_no', 'authors');
    return view('admin.static-page-contents')->with($data);
  }
  public function getData(Request $request)
  {
    $rows = StaticPageContent::where('id', '!=', '0');
  ...
  public function update($id, Request $request)
  {
    ...
    $field = StaticPageContent::find($id);
    $field->description = $request['description'];
    $field->save();
  ...
  {
    if ($id) {
      $row = StaticPageContent::findOrFail($id);
      $result = $row->delete();
      if ($result) {
