              ...
              <form id="{{ $ft == 'add' ? 'dataForm' : 'editForm' }}" {{ $ft == 'edit' ? 'action=' . $url : '' }}
                ...
                <div class="row">
                  <div class="col-md-3 col-sm-12 mb-3">
                    <x-InputField type="text" label="Status" name="status" id="statu" :ft="$ft"
                      :sd="$sd"></x-InputField>
                  </div>
                    ...
                    <div class="form-group">
                      <label>Select Color Class :
                        @foreach ($classes as $class)
                          <span class="badge bg-{{ $class }}">{{ ucwords($class) }}</span>
                        @endforeach
                      </label>
                      <select name="class" id="class" class="form-control select2">
                        ...
                        @foreach ($classes as $item)
                          <option value="{{ $item }}"
                            {{ ($ft == 'edit' && $sd->class == $item) || old('class') == $item ? 'selected' : '' }}>
                            {{ $item }}</option>
                        @endforeach
                      </select>
                  ...
                  <div class="col-md-12 col-sm-12 mb-3">
                    <x-TextareaField label="Shortnote/Explanation" name="shortnote" id="shortnote" :ft="$ft"
