      ...
      <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
          <h4 class="mb-sm-0 font-size-18">
            {{ $page_title }} <span class="text-danger">
              ({{ $examtab->getExam->exam_name }} - {{ $examtab->tab_name }})
            </span>
          </h4>
        ...
        <div class="card">
          ...
          <div class="card-body {{ $ft=='edit'?'':'hide-thi' }}" id="tblCDiv">
            <form id="{{ $ft=='add' ? 'dataForm' : 'editForm' }}" {{ $ft=='edit' ?'action='.$url:'' }} class="needs-validation" method="post" enctype="multipart/form-data" novalidate>
              @csrf
              <input type="hidden" name="tab_id" id="tab_id" value="{{ $examtab->id }}">
              <div class="row">
                <div class="col-md-12 col-sm-12 mb-3">
                  <x-SelectField label="Parent Heading" name="parent_id" id="parent_id" savev="id" showv="heading" :list="$rows" :ft="$ft" :sd="$sd"></x-SelectField>
                </div>
                <div class="col-md-12 col-sm-12 mb-3">
                  <x-InputField type="text" label="Enter Heading" name="heading" id="heading" :ft="$ft" :sd="$sd">
                  </x-InputField>
                </div>
                <div class="col-md-12 col-sm-12 mb-3">
                  <x-TextareaField label="Top Description" name="description" id="description" :ft="$ft" :sd="$sd">
                  </x-TextareaField>
                </div>
              </div>
...
<script>
  ...
  function getHeadings(){
    ...
    return new Promise(function(resolve,reject) {
      setTimeout(() => {
        $.ajax({
        url: "{{ aurl($page_route.'/get-headings') }}",
        method: "GET",
        data: {
          tab_id: tab_id,
        },
